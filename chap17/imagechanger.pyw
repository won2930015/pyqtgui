...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.imageLabel.setContextMenuPolicy(
            Qt.ActionsContextMenu)  # setContextMenuPolicy::设置_上下文_菜单_策略, ActionsContextMenu::动作_上下文_菜单.
        self.setCentralWidget(self.imageLabel)

        logDockWidget = QDockWidget("Log", self)  # QDockWidget::船坞控件
        logDockWidget.setObjectName("LogDockWidget")
        logDockWidget.setAllowedAreas(Qt.LeftDockWidgetArea |  # etAllowedAreas::设置_允许_区域
...
def main():
    ...
    if appTranslator.load("imagechanger_" + locale,
                          ":/"):  # 如设置确定国家的→→目标语言::语言[Chinese],国家/地区[China]需要设置准确文件名,imagechanger_zh_CN.qm.如搜索失败是不会搜索imagechanger_zh.qm文件的.
